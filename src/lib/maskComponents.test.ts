...
import { findMaskComponents, findMaskComponentCentroids } from './maskComponents';
...
describe('findMaskComponents', () => {
  it('returns nothing for an empty mask', () => {
    expect(findMaskComponents(emptyMask(20, 20), 20, 20)).toEqual([]);
  });

  it('a single rectangle produces one component with the expected centroid + area', () => {
    const w = 20, h = 20;
    const mask = emptyMask(w, h);
    stampRect(mask, w, 5, 5, 4, 6); // 4×6 rect at (5,5) → 24 px, centroid (6.5, 7.5)
    const comps = findMaskComponents(mask, w, h);
    expect(comps).toHaveLength(1);
    expect(comps[0].areaPx).toBe(24);
    expect(comps[0].cx).toBeCloseTo(6.5, 6);
    expect(comps[0].cy).toBeCloseTo(7.5, 6);
    expect(comps[0].pixels).toHaveLength(24);
  });

  it('three disjoint rectangles produce three components in scan order', () => {
    const w = 60, h = 30;
    const mask = emptyMask(w, h);
    stampRect(mask, w, 5,  5, 4, 4);   // top-left, area 16
    stampRect(mask, w, 30, 5, 6, 6);   // top-right, area 36
    stampRect(mask, w, 15, 18, 8, 8);  // bottom-middle, area 64
    const comps = findMaskComponents(mask, w, h);
    expect(comps).toHaveLength(3);

    // Scan order is top-left → top-right → bottom-middle.
    expect(comps[0].cx).toBeCloseTo(6.5, 6);
  ...
  it('finds components touching the edges of the canvas', () => {
    ...
    mask[w * h - 1] = 1;          // bottom-right corner
    const comps = findMaskComponents(mask, w, h);
    expect(comps).toHaveLength(2);
...
describe('findMaskComponentCentroids', () => {
  it('returns the same centroids and areas as findMaskComponents (without pixels)', () => {
    const w = 30, h = 30;
    const mask = emptyMask(w, h);
    stampRect(mask, w, 2,  2, 4, 4);
    stampRect(mask, w, 18, 18, 6, 5);
    const full  = findMaskComponents(mask, w, h);
    const light = findMaskComponentCentroids(mask, w, h);
    expect(light).toHaveLength(full.length);
    for (let i = 0; i < full.length; i++) {
      expect(light[i].cx).toBeCloseTo(full[i].cx, 6);
      expect(light[i].cy).toBeCloseTo(full[i].cy, 6);
      expect(light[i].areaPx).toBe(full[i].areaPx);
    }
  });
});
