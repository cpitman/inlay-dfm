 ...
 *
 * Polygon-native: parses the SVG directly via
 * `polygonStrokeExtraction`, which offsets each stroked element's
 * polyline by `width/2` through Clipper to produce exact stroke-
 * band geometry (no rasterization, no chord-error overshoot — the
 * 2-pixel margin the bitmap path used to fudge for is simply
 * unnecessary). The `visibleStrokesMP` from that pass drives the
 * fill subtraction; `allStrokesMP` drives the new strokeLayer.
 */

import type { Layer } from '@/types';
import { extractStrokesPolygon } from './polygonStrokeExtraction';
import {
...
export async function extractStrokeLayer(
  ...
  const { allStrokesMP, visibleStrokesMP } = extractStrokesPolygon(svgText);
  ...
  for (const layer of fillLayers) {
    ...
    // No change: keep the original fragment (preserves bezier fidelity).
